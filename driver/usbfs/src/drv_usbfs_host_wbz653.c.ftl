...
<#else>
    ...
    if(isLowSpeed)
    {
        usb->USB_UADDR |= USB_UADDR_LSPDEN_Msk;
    }
    else
    {
        usb->USB_UADDR &= ~USB_UADDR_LSPDEN_Msk;
    }
    usb->USB_UTOK = (uint32_t)((uint32_t)pid << USB_UTOK_PID_Pos) | (uint32_t)endpoint;
</#if>
}
...
<#else>
    ...
    /* Set the SOF threshold value in bit times */
    usb->USB_USOF |= USB_USOF_CNT(0x4A);

    /* Enable the VBUSON bit in the OTGCON register. Even if the actual VBUSON
     * pin is not under USB module control, this bit must be set for host
     * operation */
    usb->USB_UOTGCON |= USB_UOTGCON_VBUSON_Msk;
    ...
    /* Configure endpoint 0 control register for Host operation. */
    usb->USB_UEP[0].USB_UEPx = USB_UEPxSET_RESETVALUE;
    usb->USB_UEP[0].USB_UEPx |= (USB_UEPx_RETRYDIS_Msk | USB_UEPx_EPHSHK_Msk | USB_UEPx_EPTXEN_Msk | USB_UEPx_EPRXEN_Msk);
</#if>
...
<#if USE_PLIB == false>
            if(OSAL_MUTEX_Lock(&pUSBDrvObj->mutexID, OSAL_WAIT_FOREVER) == (OSAL_RESULT) OSAL_RESULT_TRUE)
<#else>
...
<#if core.COVERITY_SUPPRESS_DEVIATION?? && core.COVERITY_SUPPRESS_DEVIATION>
#pragma coverity compliance block \
(deviate:12 "MISRA C-2012 Rule 11.6" "H3_USB_MISRAC_2012_R_11_6_DR_1" )\
        ...
        switch(pirp->tempState)
        {
            case DRV_USBFS_HOST_IRP_STATE_SETUP_STAGE:
                ...
                if((((uint8_t *)(pirp->setup))[0] & 0x80U) != 0U)
                {
                    ...
                    pipe->endpointAndDirection |= (uint8_t)((uint32_t)USB_DATA_DIRECTION_DEVICE_TO_HOST << 7);
                }
                else
                {
                    /* Data stage direction is from host to device */
                    pipe->endpointAndDirection &= 0xFU;
                    pipe->endpointAndDirection |= (uint8_t)((uint32_t)USB_DATA_DIRECTION_HOST_TO_DEVICE << 7);
                }
...
<#else>
                pBDT->word[1] = (uint32_t)(pirp->setup);
</#if>
...
<#else>
                usb->USB_UEP[0].USB_UEPx |= USB_UEPx_EPHSHK_Msk;
</#if>
            ...
            for(pipeCount = 0; pipeCount < DRV_USBFS_HOST_PIPES_NUMBER; pipeCount++)
            {
                ...
                if(gDrvUSBHostPipeObj[pipeCount].inUse == false)
                {
                    ...
                    if(bitSetCount != 10U)
                    {
                        ...
                        {
                            if (((pPipe->pipeType == USB_TRANSFER_TYPE_INTERRUPT) || (pPipe->pipeType == USB_TRANSFER_TYPE_CONTROL)) && (bitSetCount < 4U))
                            {
...
<#else>
    pBDT->word[1] = (uint32_t)((uint8_t *)pirp->data + pirp->completedBytes);
</#if>
...
<#if USE_PLIB == true>
    PLIB_USB_EPnHandshakeEnable(usbID, 0);
<#else>
    usb->USB_UEP[endpoint].USB_UEPx |= USB_UEPx_EPHSHK_Msk;
</#if>
...
<#else>
    uint32_t usbInterrupts;
    uint32_t enabledUSBInterrupts;
</#if>
...
<#else>
    usbInterrupts = (usb->USB_UIR);
    enabledUSBInterrupts = (usb->USB_UIE);
    readInterruptFalgGet = (usb->USB_UOTGIE & USB_UOTGIE_ONE_MSEC_Msk) != 0U;
</#if>
<#if USE_PLIB == true>
...
<#else>
    if (((usb->USB_UOTGIR & USB_UOTGIR_T1MSECIF_Msk) != 0U) && readInterruptFalgGet)
    {
        usb->USB_UOTGIR = USB_UOTGIR_T1MSECIF_Msk;
</#if>
...
<#else>
                usb->USB_UOTGIE &= (~USB_UOTGIE_ONE_MSEC_Msk);
</#if>
                pusbdrvObj->isAttachDebouncing = false;
...
<#else>
                if ((usb->USB_UIR & USB_UIR_ATTACHIF_Msk) != 0U)
</#if>
                {
...
<#else>
                    ...
                    usb->USB_UIE |= (USB_UIE_URSTIE_DETACHIE_Msk);
                    usb->USB_UIE |= (USB_UIE_TRNIE_Msk);
</#if>
...
<#else>
                    usb->USB_UIE |= USB_UIE_ATTACHIE_Msk;
</#if>
                }
            }
        }
...
<#else>
                usb->USB_UCON &= (~USB_UCON_USBRST_Msk);
</#if>
            }
...
<#else>
                if ((usb->USB_UCON & USB_UCON_JSTATE_Msk) == 0U)
</#if>
                {
...
<#else>
                    usb->USB_UADDR |= USB_UADDR_LSPDEN_Msk;
                    usb->USB_UEP[0].USB_UEPxSET = USB_UEPx_LSPD_Msk;
</#if>
                }
                else
                {
...
<#else>
                    usb->USB_UADDR &= (~USB_UADDR_LSPDEN_Msk);
                    usb->USB_UEP[0].USB_UEPxCLR = USB_UEPx_LSPD_Msk;
</#if>
                }
...
<#if USE_PLIB == true>
                ...
                PLIB_USB_InterruptEnable(usbID, USB_INT_ATTACH);
<#else>
                usb->USB_UIR = USB_UIR_ATTACHIF_Msk;
                usb->USB_UIE |= USB_UIE_ATTACHIE_Msk;
</#if>
            }
        }
...
<#else>
    /* Check if an error has occurred */
    if (((usbInterrupts & USB_UIR_UERRIF_Msk) != 0U) && ((enabledUSBInterrupts & USB_UIE_UERRIE_Msk) != 0U))
    {
        uint32_t errorType;

        errorType = usb->USB_UEIR;
...
<#else>
    if (((usbInterrupts & USB_UIR_SOFIF_Msk) != 0U) && ((enabledUSBInterrupts & USB_UIE_SOFIE_Msk) != 0U))
    {
        /* SOF threshold reached by Host. Call the transaction scheduler so that
         * we schedule the next set of transfers */
...
<#else>
    if (((usbInterrupts & USB_UIR_STALLIF_Msk) != 0U) && ((enabledUSBInterrupts & USB_UIE_STALLIE_Msk) != 0U))
    {
        /* The device stalled the request. At this point we don't know yet what
         * to do with this interrupt. Clearing the flag for now */
...
<#else>
    if (((usbInterrupts & USB_UIR_RESUMEIF_Msk) != 0U ) && ((enabledUSBInterrupts & USB_UIE_RESUMEIE_Msk) != 0U))
    {
        /* Device sent resume signalling to the host */
...
<#else>
    if (((usbInterrupts & USB_UIR_IDLEIF_Msk) != 0U ) && ((enabledUSBInterrupts & USB_UIE_IDLEIE_Msk) != 0U))
    {
...
<#else>
    if (((usbInterrupts & USB_UIR_ATTACHIF_Msk) != 0U ) && ((enabledUSBInterrupts & USB_UIE_ATTACHIE_Msk) != 0U))
    {
        /* Host got a device attach */
        usb->USB_UIR  = USB_UIR_ATTACHIF_Msk;
...
<#if USE_PLIB == true>
    if((((uint32_t)usbInterrupts & (uint32_t)USB_INT_HOST_DETACH) != 0U) && (((uint32_t)enabledUSBInterrupts & (uint32_t)USB_INT_HOST_DETACH) != 0U))
    {
        ...
        PLIB_USB_InterruptFlagClear(usbID, USB_INT_HOST_DETACH);
<#else>
    if(((usbInterrupts & USB_UIR_URSTIF_DETACHIF_Msk) != 0U ) && ((enabledUSBInterrupts & USB_UIE_URSTIE_DETACHIE_Msk) != 0U))
    {
        /* Host got a device detach */
        usb->USB_UIR  = USB_UIR_URSTIF_DETACHIF_Msk;
</#if>
...
<#if USE_PLIB == true>
    if(PLIB_USB_InterruptFlagGet(usbID, USB_INT_TOKEN_DONE) && ((((uint32_t)enabledUSBInterrupts) & ((uint32_t)USB_INT_TOKEN_DONE)) != 0U))
    {
        ...
        PLIB_USB_InterruptFlagClear(usbID,USB_INT_TOKEN_DONE);
<#else>
    if (((usb->USB_UIR & USB_UIR_TRNIF_Msk) != 0U) && ((enabledUSBInterrupts & USB_UIR_TRNIF_Msk) != 0U ))
    {
        /* Get the last transaction status */
        lastDirection = (USB_BUFFER_DIRECTION)(((usb->USB_USTAT & USB_USTAT_DIR_Msk) != 0U) ? 1 : 0);
        lastPingPong = (USB_PING_PONG_STATE )(((usb->USB_USTAT & USB_USTAT_PPBI_Msk) != 0U) ? 1 : 0);
